...
# 1. 单次淡入（进入主界面时）
transform main_fade_in:
    alpha 0.0          # 初始：完全透明
    linear 1.5 alpha 1.0  # 1.5秒平滑淡入（数字越小速度越快）

# 2. 循环淡入淡出（呼吸效果，无限循环）
transform main_fade_loop:
    alpha 0.0
    linear 2.0 alpha 1.0  # 2秒淡入
    linear 2.0 alpha 0.0  # 2秒淡出
    repeat               # 无限循环

# 3. 退出淡出（离开主界面时）
transform main_fade_out:
    alpha 1.0
    linear 1.0 alpha 0.0  # 1秒淡出
...
screen main_menu():
    ...
    add gui.main_menu_background#主界面图（切换节奏在 script.rpy 的 cycling_main_menu_bg ATL 中）
